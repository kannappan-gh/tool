...
def check_ssl():
    ...
    ssl = None
    if path.exists("/discovery/etc/hadoop/conf/core-site.xml"):
        hadoop_path = "/discovery/etc/hadoop/conf/core-site.xml"
    elif path.exists("/discovery/etc/hive/conf/core-site.xml"):
        hadoop_path = "/discovery/etc/hive/conf/core-site.xml"
    else:
        return ssl
...
def check_config_path():
    ...
    config_path = {}
    if path.exists("/discovery/etc/hadoop/conf/core-site.xml"):
        config_path["core"] = "/discovery/etc/hadoop/conf/core-site.xml"
    else:
        if path.exists("/discovery/etc/hive/conf/core-site.xml"):
            config_path["core"] = "/discovery/etc/hive/conf/core-site.xml"
        else:
            config_path["core"] = None
    if path.exists("/discovery/etc/hadoop/conf/yarn-site.xml"):
        config_path["yarn"] = "/discovery/etc/hadoop/conf/yarn-site.xml"
    else:
        if path.exists("/discovery/etc/hive/conf/yarn-site.xml"):
            config_path["yarn"] = "/discovery/etc/hive/conf/yarn-site.xml"
        else:
            config_path["yarn"] = None
    if path.exists("/discovery/etc/hadoop/conf/mapred-site.xml"):
        config_path["mapred"] = "/discovery/etc/hadoop/conf/mapred-site.xml"
    else:
        if path.exists("/discovery/etc/hive/conf/mapred-site.xml"):
            config_path["mapred"] = "/discovery/etc/hive/conf/mapred-site.xml"
        else:
            config_path["mapred"] = None
    if path.exists("/discovery/etc/hadoop/conf/hdfs-site.xml"):
        config_path["hdfs"] = "/discovery/etc/hadoop/conf/hdfs-site.xml"
    else:
        if path.exists("/discovery/etc/hive/conf/hdfs-site.xml"):
            config_path["hdfs"] = "/discovery/etc/hive/conf/hdfs-site.xml"
        else:
            config_path["hdfs"] = None
    if path.exists("/discovery/etc/hive/conf/hive-site.xml"):
        config_path["hive"] = "/discovery/etc/hive/conf/hive-site.xml"
    else:
        config_path["hive"] = None
    if path.exists("/discovery/etc/spark/conf/spark-defaults.conf"):
        config_path["spark"] = "/discovery/etc/spark/conf/spark-defaults.conf"
    else:
        config_path["spark"] = None
    if path.exists("/discovery/etc/kafka/conf/kafka-client.conf"):
        config_path["kafka"] = "/discovery/etc/kafka/conf/kafka-client.conf"
    else:
                    ...
                    if n.isnumeric():
                        ...
                        for i in range(0, n):
                            ...
                            while c2 > 0:
                                ...
                                t1 = input()
                                if t1 in ["y", "Y"]:
                                    broker["log_dir"] = "/discovery/var/local/kafka/data/"
                                    break
                                elif t1 in ["n", "N"]:
                                    print(
...
def upload_bucket(JsonCredentials, log_pdf_files):
    storage_client = storage.Client.from_service_account_json(JsonCredentials)
    #Get bucket name
    bucket = storage_client.get_bucket('hadoop-discovery')

    recommendation_filePath = "hadoop_input_files/"
        
    if log_pdf_files.find('.pdf')!=-1:
        recommendation_filePath = recommendation_filePath+"pdf_files"
        filename = "%s/%s" % (recommendation_filePath, log_pdf_files.split('/')[-1])
        blob = bucket.blob(filename)
        with open(log_pdf_files, 'rb') as txt_file:
            blob.upload_from_file(txt_file)

    elif log_pdf_files.find('.log')!=-1:
        recommendation_filePath = recommendation_filePath+"log_files"
        filename = "%s/%s" % (recommendation_filePath, log_pdf_files.split('/')[-1])
        blob = bucket.blob(filename)
        with open(log_pdf_files, 'rb') as txt_file:
            blob.upload_from_file(txt_file)
